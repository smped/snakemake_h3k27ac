...
```{r set-fig.path}
comp_name <- glue(
  "{names(pairs)[[1]]}_{pairs[[1]][[1]]}_{pairs[[1]][[2]]}-", 
  "{names(pairs)[[2]]}_{pairs[[2]][[1]]}_{pairs[[2]][[2]]}"
)
...
gs_method <- ifelse(mean(pwf_mapped$DEgenes) > 0.8, "Hypergeometric", "Wallenius")
...
`r if (gs_method == "Hypergeometric") "No sampling bias offset was incorporated, instead using a simple Hypergeometric model for enrichment testing"`
`r if (gs_method == "Wallenius") "Gene-length was included as the source of any potential sampling bias for a gene being mapped to a source-bound range."`
...
gs_method <- ifelse(mean(pwf_t1t2$DEgenes) > 0.8, "Hypergeometric", "Wallenius")
if (sum(pwf_t1t2$DEgenes) > min_genes & sum(pwf_t1t2$DEgenes) < max_genes) {
  goseq_t1t2_res <- pwf_t1t2 %>% 
    goseq(gene2cat = gs_by_geneid, gs_method) %>%  
    dplyr::filter(numDEInCat > 0) %>% 
    as_tibble() %>% 
    dplyr::select(
      gs_name = category, pval = over_represented_pvalue,
...
goseq_group_res <- group_ids %>% 
  mclapply(
    function(x) {
      ...
      gs_method <- ifelse(mean(pwf$DEgenes) > 0.8, "Hypergeometric", "Wallenius")
...
However, if the number of genes in the test set exceeded 80% of genes in the 'universe', no sampling bias was included, due to issues with extreme values in the current R implementation of Wallenius' Non-Central Hypergeometric Distribution.
