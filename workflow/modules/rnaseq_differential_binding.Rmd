...
### Detected Genes By `r source` Binding Region

Firstly, the distribution of merged windows, where `r source` was considered as present, was compared to genes within the RNA-Seq dataset.
...
```{r plot-region-detected, fig.height = 6, fig.cap = glue("*Distribution of {source}-bound windows by region, according to whether the window is mapped to a detected gene in the RNA-Seq dataset.*")}
w <- 0.8
min_p <- 0.05
merged_results %>%
  mutate(
...
dir_gs <- merged_results %>% 
  ...
  as_tibble() %>% 
  mutate(group = glue("H3K27ac {status}")) %>% 
  split(.$group)%>% 
  lapply(pull, "gene_id") %>% 
  lapply(unlist) %>% 
  lapply(unique) %>% 
...
reg_dir_gs <- merged_results %>% 
  ...
  as_tibble() %>% 
  mutate(group = glue("{region} / H3K27ac {status}")) %>% 
  split(.$group)%>% 
  lapply(pull, "gene_id") %>% 
  lapply(unlist) %>% 
  lapply(unique) %>% 
...
### DE Genes Associated with Changed `r source` Binding
...
`r if (nrow(de_genes_db_regions) == 0) "No differentially expressed genes were common to differentially-bound regions"`
...
cp <-  htmltools::em(
  glue(
    "
    Differentially Expressed (DE) genes were checked for H3K27ac signal
    ...
    genes. The full set of ranges considered to be showing eveidence of altered 
    H3K27ac signal are shown for each gene. The summarised range containing 
    all mapped ranges is given in the collapsed row, along with the range of 
    distances to gene and the range of signal levels. In the cllapsed row, the 
    value for logFC represents the average aross all mapped regions, with the 
    maximum FDR also shown. If H3K27ac signal occurs within another gene this 
  ...
  reactable(
    ...
    columns = c(
      list(
        ...
        ChIP_logCPM = colDef(
          name = paste(source, "signal (logCPM)"),
          maxWidth = fs*6,
          aggregate = range_js,
          format = colFormat(digits = 2),
...
### P-Values By `r source` Status

```{r plot-p-source, fig.height = 6, fig.cap = glue("*P-Values for differential expression partitioned by {source} ChIP peak status. Genes were considered as being mapped to a differentially bound peak if one or more peaks was considered as differentially bound.*")}
y <- ggplot_build(
...
rnaseq %>% 
  mutate(
    mapped = case_when(
      gene_id %in% unlist(diff_windows[names(diff_windows) %in% c("Up", "Down")]) ~ "H3K27ac Differential Signal",
      gene_id %in% unlist(diff_windows[names(diff_windows) %in% c("Unchanged")]) ~ "H3K27ac Unchanged",
      TRUE ~ "No H3K27ac Detected"
    )
  ) %>% 
  ggplot(
...
`r if (nrow(gsea_dir_sig) == 0) glue("No association was found between H3K27ac signal and *directional* significance in differentially expressed genes.")`
...
cp <-  htmltools::em(
  glue(
    ...
    "the direction of H3K27ac signal change with ",
...
tbl <- gsea_dir_sig %>%
  mutate(
    ...
    H3K27ac := str_extract(pathway, "(Up|Down|Unchanged)$"),
    pathway = str_remove_all(pathway, " / [^/]+$")
  ) %>% 
  dplyr::select(
    H3K27ac, Windows = size, Direction, 
...
`r if (nrow(gsea_reg_dir_sig) == 0) glue("No association was found between H3K27ac signal and *directional* significance in differentially expressed genes, when incorporating genomic region.")`
...
cp <-  htmltools::em(
  glue(
    ...
    "region was the best overlap, and the direction of H3K27ac signal with ",
...
tbl <- gsea_reg_dir_sig %>%
  ...
  dplyr::select(
    pathway, H3K27ac, Windows = size, Direction, 
    p = pval, FDR = padj, `Edge Size`, `Leading Edge` = leadingEdge
  ) %>% 
  reactable(
    filterable = TRUE,
...
`r if (nrow(gsea_nondir_sig) == 0) glue("No association was found between H3K27ac signal and *overall* significance in differentially expressed genes.")`
...
cp <-  htmltools::em(
  glue(
    "Merged windows were mapped to genes and their position amongst the ",
    "RNA-Seq results was assessed. Windows were classified based on  ",
    "the direction of H3K27ac signal with ",
...
tbl <- gsea_nondir_sig %>%
  ...
  dplyr::select(
    H3K27ac, Windows = size, p = pval, FDR = padj, 
...
`r if (nrow(gsea_reg_nondir_sig) == 0) glue("No association was found between H3K27ac signal and *overall* significance in differentially expressed genes.")`
...
tbl <- gsea_reg_nondir_sig %>%
  ...
  dplyr::select(
    pathway, H3K27ac, Windows = size, p = pval, FDR = padj, 
    `Edge Size`, `Leading Edge` = leadingEdge
  ) %>% 
  reactable(
    filterable = TRUE,
...
Using the ranked list of genes from the RNA-Seq data, the 5 genes most highly ranked for differential H3K27ac signal, with more than one window mapped to it were selected for visualisation.
...
.makeDEGCaption <- function(id) {
  ...
  glue(
    "*Region showing all merged H3K27ac windows mapped to ", 
    "{nm}. Windows considered to be unchanged for {source} are ", 
    "annotated in grey, with other colours indicating {source} gain or loss. ",
    "Mapped windows are only shown if within 5Mb of {nm}. ",
    ifelse(
      has_hic,
      glue(
        "HiC Interactions are only shown which directly connect the H3K27ac bound region to {nm}. "
      ),
      glue("")
    ),
    "Gene-centric regions are shown in the top panel. ", 
    "The estimated logFC for {nm} is ", sprintf("%.2f ", df$logFC),
...
The enrichment testing results from the `r source` differential binding analysis were then
...
`r if (length(cmn_gs) == 0) glue("No gene sets were shared between all {source} differentially bound windows and the RNA-Seq data")`
...
### Windows With Increased `r source`
...
`r if (length(cmn_gs) == 0) glue("No gene sets were shared between {source} gained windows and the RNA-Seq data")`

```{r tbl-cmn-up-gs, eval = length(cmn_gs) > 0, echo = length(cmn_gs) > 0}
cp <- htmltools::tags$em(
  glue(
    "
    Genes mapped to all windows showing evidence of increased H3K27ac signal 
...
### Windows With Decreased `r source`
...
`r if (length(cmn_gs) == 0) glue("No gene sets were shared between {source} decreased windows and the RNA-Seq data")`

```{r tbl-cmn-down-gs, eval = length(cmn_gs) > 0, echo = length(cmn_gs) > 0}
cp <- htmltools::tags$em(
  glue(
    "
    Genes mapped to all windows showing evidence of decreased H3K27ac signal 
    were tested for enrichment using goseq, and results were compared to GSEA 
    results for the RNA-Seq dataset. The {comma(length(cmn_gs), 1)} Gene Sets 
    found in both analyses are detailed below. Leading Edge genes represent 
    the genes as ranked in the provided RNA-Seq dataset.
