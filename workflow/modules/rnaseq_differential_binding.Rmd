...
Using the ranked list of genes from the RNA-Seq data, the 5 genes most highly ranked for differential expression, with more than one `r source`-bound window mapped to it were selected for visualisation.
...
.makeDEGCaption <- function(id) {
  ...
  glue(
    "*Region showing all merged {source}-bound windows mapped to ", 
    ...
    ifelse(
      has_hic,
      glue(
        "HiC Interactions are only shown which directly connect {source} to {nm}. "
      ),
      glue("")
    ),
