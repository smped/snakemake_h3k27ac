...
consensus_peaks <- oracle_peaks %>%
  ...
  mutate(
    ...
    region = bestOverlap(
      ., 
      lapply(gene_regions, plyranges::select, region) %>% 
        GRangesList() %>% 
        unlist(),
      var = "region"
    ) %>% 
  ...
  plyranges::select(-keep, -peak_position)
```
...
if (any_detected){
  ...
  if (!is.na(rna_lfc_col) & !is.na(rna_fdr_col)) {
    hfgc_genes <- hfgc_genes %>%
      ...
      splitAsList(.$status) %>%
      lapply(plyranges::select, -status) %>%
      GRangesList()
...
df <- gene_regions %>% 
  ...
  lapply(as_tibble) %>% 
  lapply(dplyr::select, region, marked, detected) %>% 
  bind_rows(.id = "name") %>% 
...
names(oracle_peaks) %>% 
  lapply(
    function(x) {
      ...
      oracle_peaks[[x]] %>% 
        subset(keep) %>% 
        plyranges::select(score, signalValue, pValue, qValue, peak) %>% 
