...
out_path <- here::here("output", "differential_binding", source)
...
```{r load-hic}
hic <- GInteractions()
hic_path <- here::here(config$paths$hic[[source]])
has_hic <- FALSE
...
facet_labeller <- as_labeller(
  c(
    ...
    c(
      Up = glue("Increased {source} Binding"),
      Down = glue("Decreased {source} Binding")
    ),
    c(
...
all_out <- list(
  results = file.path(
    out_path, 
    glue("{source}_{treat_levels[[1]]}_{treat_levels[[2]]}-differential_binding.rds")
  ),
  csv = file.path(
    out_path, 
    glue("{source}_{treat_levels[[1]]}_{treat_levels[[2]]}-differential_binding.csv.gz")
  ),
  windows  = file.path(
    out_path,
    glue("{source}_{treat_levels[[1]]}_{treat_levels[[2]]}-filtered_windows.rds")
  ),
  up_regions = file.path(
    out_path, glue("{source}_{treat_levels[[1]]}_{treat_levels[[2]]}-up.bed")
  ),
  down_regions = file.path(
    out_path, glue("{source}_{treat_levels[[1]]}_{treat_levels[[2]]}-down.bed")
  ),
  de_genes = file.path(
    out_path, glue("{source}_{treat_levels[1]}_{treat_levels[2]}-DE_genes.csv")
  ),
  renv = here::here(
    "output/envs",
    glue(
      "{source}_{treat_levels[[1]]}_{treat_levels[[2]]}-differential_binding.RData"
    )
  )
)
```
...
1. Genes mapped to any window with detected `r source` are compared to all genes not mapped to any window
...
3. Genes mapped to windows with *increasing* `r source` binding are compared to genes mapped to windows which are not differentially bound
4. Genes mapped to windows with *decreasing* `r source` binding are compared to genes mapped to windows which are not differentially bound
...
colData(window_counts) <- colData(window_counts) %>% 
  ...
  mutate(
    ...
    source = str_replace_na(source, "Input")
  ) %>% 
  as.data.frame() %>% 
...
With the exclusion of black-listed regions, all alignments within each window were counted for each `r source`-associated sample, and all relevant input samples.
...
```{r plot-density-windows, fig.cap = "*Density plot for __all windows prior to filtering__, and the selection of those more likely to contain true signal.*"}
plotAssayDensities(
  ...
  labs(
    x = "log(counts + 1)", y = "Density", colour = "Treat", linetype = "source"
  )
```

### logCPM Densities (Pre-SQN)

```{r plot-logcpm-densities, fig.cap = "*logCPM distributions for all retained windows __prior__ to smooth quantile normalisation*"}
filtered_counts %>% 
...
```{r plot-rle-normalised, fig.height=6, fig.cap = glue("*RLE plot showing logCPM values after Smooth Quantile normalisation. RLE values were calculated within each treatment group to account for the potentially different binding dynamics of {source}.*")}
filtered_counts %>% 
  plotAssayRle(
...
```{r plot-pca, fig.height = 5, fig.cap = "*PCA plots for logCPM values A) before and B) after Smooth Quantile normalisaton*"}
...
A simple linear model was fitted taking `r treat_levels[[1]]` as the baseline and estimating the effects of `r treat_levels[[2]]` on `r source` binding within each sliding window, incorporating a trended prior-variance.
...
H_A: |\mu| > \lambda
$$
where $\mu$ represents the true mean change in `r source` binding.
...
For subsequent classification when combining across multiple differential binding results, an additional p-value testing $\mu = 0$ was added, however this is not used for detection of sites showing evidence of altered `r source` binding.
...
```{r plot-merged-size, fig.height=7, fig.cap = glue("*Comparison of {source} signal intensity (logCPM) and merged window size (kb) for windows which overlap a macs2 consensus peak and those which don't. A mall amount of jitter has been added to the y-axis in the lower-left panel.*")}
merged_results %>% 
  mutate(kb = width / 1e3) %>% 
  as_tibble() %>% 
  mutate(
  ...
  labs(
    ...
    fill = glue("{source} Macs2 Peak"), colour = glue("{source} Macs2 Peak")
  ) +
  theme(
...
merged_results <- merged_results %>%
  mapByFeature(
    genes = genes_gr, 
    prom = granges(gene_regions$promoters),
    gi = hic, 
...
For visualisation purposes, only genes which were considered as detected in any provided RNA-Seq data will be shown as the mapping sources.
...
```{r plot-volcano, fig.cap = glue("*Volcano plot showing regions with evidence of differential {source} binding. Regions overlapping a consensus peak are coloured separately to those which are not. The most significant regions are labelled along with any genes these regions are mapped to.*")}
merged_results %>% 
  as_tibble() %>% 
...
```{r plot-down-heatmap, eval = "Down" %in% names(profile_heatmaps), echo = "Down" %in% names(profile_heatmaps), fig.cap = glue("*Heatmap and histogram for all regions considered to show evidence of decreased {source} binding in the {treat_levels[[2]]} group, separated by those which did or did not overlap a consensus peak. A total of {comma(length(sig_ranges[['Down']]))} regions were in this group.*"), fig.height = min(16, length(sig_ranges$Down)/1.5e3 + 6)}
...
```{r plot-db-regions, fig.height = 7, fig.cap = glue("*Merged windows considered as showing differential {source} binding across all genomic regions.*")}
merged_results %>% 
...
cp <- htmltools::em(
  glue(
    "Overall results for changed {source} binding after {treat_levels[[2]]} ",
...
line_col <- list2("{source}" := treat_colours)
...
makeCaption <- function(.gr) {
  ...
  cp <- c(
    glue(
      "*The {width(.gr)}bp region showing {dir} {source} binding in response to ", 
...
msigdb %>% 
  ...
  mutate(
    Description = case_when(
      ...
      str_detect(`Sub-Category`, "TFT") ~ str_replace_all(`Sub-Category`, "TFT:(.+)", "Transcription Factor source Prediction Gene-Sets: \\1"),
      str_detect(`Sub-Category`, "MIR") ~ str_replace_all(`Sub-Category`, "MIR:(.+)", "microRNA source Gene-Sets: \\1"),
...
## Windows With Detected `r source`
...
cp <- htmltools::tags$caption(
  htmltools::em(
    ...
    htmltools::em(
      glue(
        "
         < {enrich_alpha}) amongst the merged windows with detectable {source} 
...
## Windows With Differentially Bound `r source`
  ...
  htmltools::em(
    ...
    htmltools::em(
      glue(
        ...
        {source} binding. Genes mapped to a differentially-bound window were 
...
## Windows With Increased `r source` Binding
  ...
  htmltools::em(
    ...
    htmltools::em(
      glue(
        ...
        {source} binding. Genes mapped to an increasing window were 
        compared to those mapped to any merged window.
        "
      )
    )
...
## Windows With Decreased `r source` Binding
  ...
  htmltools::em(
    ...
    htmltools::em(
      glue(
        ...
        {source} binding. Genes mapped to a decreasing window were 
        compared to those mapped to any merged window.
        "
      )
    )
